...
**Aggregation Row Precedence**: When a Treasury Bulletin table contains BOTH (a) individual monthly breakdowns AND (b) an explicit aggregated annual/summary row (e.g., "Fiscal yr.", "Calendar yr.", "Annual total", "to date" row), you MUST prefer the aggregated row as the authoritative answer. Summing individual monthly values independently introduces compounding rounding errors — each sub-column row may round differently (e.g., ±1–2 per row), so the sum of monthly "Total" columns will NOT match the Calendar yr. row. The Calendar yr. row IS the "reported value" for the annual total. Only sum individual monthly values when (1) no aggregated row exists, OR (2) the question EXPLICITLY specifies to compute a custom sum (e.g., "sum only the even-numbered months"). The phrase "use only the reported values for individual calendar months" in a question means "use the 1953 period data (which is reported monthly) rather than a fiscal year figure" — it does NOT mean "ignore the table's own aggregated row and manually sum the monthly Total column." The table structure is hierarchical: the Calendar yr. / Fiscal yr. row is the official annual sum with per-row rounding corrections, while monthly values have independent rounding that compounds when summed.
